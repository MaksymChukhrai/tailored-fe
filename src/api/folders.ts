...
export function useCreateFolder() {
  ...
  return useMutation({
    ...
    onSuccess: (folder) => {
      if (folder.parentId) {
        void queryClient.invalidateQueries({ queryKey: FOLDER_KEY(folder.parentId) })
      } else {
...
export function useRenameFolder() {
  ...
  return useMutation({
    ...
    onSuccess: (_folder, variables) => {
      ...
      void queryClient.invalidateQueries({ queryKey: FOLDER_KEY(variables.id) })
    },
  })
}
...
export function useMoveFolder() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      invalidateParent(queryClient, variables.sourceParentId, variables.dataRoomId)
      if (variables.targetParentId) {
        void queryClient.invalidateQueries({ queryKey: FOLDER_KEY(variables.targetParentId) })
      } else {
        void queryClient.invalidateQueries({
...
function invalidateParent(
  ...
  if (parentId) {
    void queryClient.invalidateQueries({ queryKey: FOLDER_KEY(parentId) })
  } else {
